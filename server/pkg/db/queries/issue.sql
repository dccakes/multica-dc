...
SELECT id, workspace_id, title, status, priority,
       ...
       estimated_hours, estimate_source
FROM issue
...
INSERT INTO issue (
    ...
    sqlc.narg('estimated_hours'), sqlc.narg('estimate_source')
) RETURNING *;
...
UPDATE issue SET
    ...
    estimated_hours = sqlc.narg('estimated_hours'),
    estimate_source = sqlc.narg('estimate_source'),
    updated_at = now()
WHERE id = $1
RETURNING *;
